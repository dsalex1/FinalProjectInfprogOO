...
public abstract class Model {
	...
	/**
	 * A hashmap of lists for blacklisting plugins for specific events after they
	 * throw an exception.
	 */
	...
	/**
	 * A method meant to be executed after all plugins have been loaded into the
	 * Model.
	 */
	public void start() {
	...
	public void registerTileType(String identifier, TileType tileType) {
		if (tileTypeDictionary.containsKey(identifier))
			throw new IllegalArgumentException(
					"The identifier '" + identifier + "' is already present in the tile mapping.");
	 ...
	 * @throes IllegalArgumentException if no tile type with the given identifier
	 *         was found
	 */
	...
	public void registerLevelType(String identifier, LevelType levelType) {
		if (levelTypeDictionary.containsKey(identifier))
			throw new IllegalArgumentException(
					"The identifier '" + identifier + "' is already present in the level type mapping.");
	 ...
	 * 
	 * @param id the identifier to lookup.
	 * @return the found level type with the given identifier (or null if not
	 *         found).
	 */
	 ...
	 * @throes IllegalArgumentException if no level type with the given identifier
	 *         was found
	 */
	...
	/**
	 * Register, initialize and load a plugin into this model. Also trigger an event
	 * that the plugin has been loaded globally.
	 * 
	...
	public void registerPlugin(Plugin plugin) {
		...
		try {
			...
			// Ignore UnsupportedOperationException's silently (most likely caused by a
			// JavaScript plugin not having implemented this method)
		} catch (Exception e) {
	...
	public void call(String eventName, Class<? extends Plugin> from, Class<?>[] cparams, Object[] params) {
		for (Plugin plugin : plugins) { // process each plugin for the event
			try {
				if (!from.isInstance(plugin))
					continue; // don't process plugins that do not implement the origin or any extending class
								// of it
				if (blackList.computeIfAbsent(eventName, k -> new ArrayList<Plugin>()).contains(plugin))
					continue; // don't process plugins that have been blacklisted for this event (also create
								// new array list if the event doesn't exist in the blacklist yet)
				plugin.getClass().getMethod(eventName, cparams).invoke(plugin, params); // call the event method through
																						// javas reflect framework
			} catch (InvocationTargetException ns) {
				if (ns.getTargetException() != null
						&& ns.getTargetException() instanceof UnsupportedOperationException) {
					blackList.get(eventName).add(plugin); // all plugins that are written in JavaScript and don't
															// implement needed interface methods land here, other
															// Plugins may use this as well
				} else
					new RuntimeException(plugin.getName(), ns.getTargetException()).printStackTrace(); // unhull the
																										// exception and
																										// output it
																										// with the
																										// associated
				...
				new RuntimeException(plugin.getName(), e).printStackTrace(); // all other exceptions should be send
																				// directly to the error stream
			}
		}
	}
